...
def plot_success_rate_vs_circuit_depth_boxplot(df1, df2, bin_size=5):
    ...
    Style matches plots 1 and 1b with side-by-side comparison.
    ...
    # Prepare data for boxplots (side by side like plots 1 and 1b)
    ibm_data = []
    rigetti_data = []
    valid_depth_groups = []
    ...
    for depth_group in depth_groups:
        ...
        if len(ibm_subset) > 0 or len(rigetti_subset) > 0:
            # Success rate is already in 0-1 scale from load_circuit_depth_datasets()
    ...
    # Create positions for boxplots (side by side like plot 1 and 1b)
    ...
    # Create boxplots matching plot 1 and 1b style
    ...
    # Set axis labels (matching plot 1 and 1b)
    ...
    # Add grid (matching plot 1 and 1b)
    ax.grid(True, linestyle='--', alpha=0.7, axis='y')
    
    # Create simple custom legend (matching plot 1 and 1b style)
    legend_elements = [
...
def run_success_rate_circuit_depth_analysis():
    ...
    # Save the plot
    output_file = os.path.join(output_dir, '3_success_rate_vs_circuit_depth_boxplot.png')
