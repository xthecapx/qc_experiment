...
\begin{table}[ht]
\centering
\caption{Correlation Statistics Between Number of Gates and Success Rate for Gate Counts \ensuremath{\leq} 70}
\label{tab:gate_correlation_stats}
\begin{tabular}{|l|c|c|}
    \hline
    \textbf{Statistic} & \textbf{IBM (\ensuremath{n=289})} & \textbf{Rigetti (\ensuremath{n=179})} \\
    \hline
    Pearson r & -0.467 & -0.558 \\
    \hline
    Spearman \ensuremath{\rho} & -0.554 & -0.568 \\
    \hline
    R\ensuremath{^{2}} & 0.218 & 0.311 \\
    \hline
    Slope & -0.007 & -0.009 \\
    \hline
    p-value & \ensuremath{6.04 \times 10^{-17}} & \ensuremath{5.09 \times 10^{-16}} \\
    \hline
\end{tabular}
\end{table}
...
The experimental results at this stage corroborate our initial hypothesis (\ref{h1}). The gate count analysis confirms a moderate negative correlation between gate count and success. Once payload size increases, success rates drop sharply regardless of gate count, indicating that payload size remains the dominant limiting factor even within the lower gate ranges. This pattern holds across both hardware platforms, reinforcing that the modest gate count effect remains secondary compared to the payload-size effect.

The moderate gate count effect can be attributed to the inherently high fidelity of single-qubit operations \cite{shuklaCompleteCharacterizationDirectlya_2020}, which display high fidelity in today's quantum hardware. Data supporting this section can be accessed at \url{https://github.com/xthecapx/qc_experiment/tree/main/experiment/ieee_analysis}.
