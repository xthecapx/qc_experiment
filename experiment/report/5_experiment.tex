...
Tables~\ref{tab:vtp_success_rates_ibm} and~\ref{tab:vtp_success_rates_rigetti} summarize the success rate statistics for vTP after executing circuits with different gate count ranges in IBM Sherbrooke and Rigetti Ankaa-3, respectively, while Figure~\ref{fig:random_gates_success_rate} presents the same data visually through boxplots. As can be seen, grouping the data by gate counts produces low mean success rates in both platforms; however, it should be noted that some of the jobs in each group achieve higher success rates such as 87.4\% for IBM Sherbrooke and 90.0\% for Rigetti Ankaa-3, which give us a hint to explore data with another level of grouping.

\begin{figure}[ht]
    \centering
    \includegraphics[width=1\linewidth]{img/2b_success_rate_vs_gates_grouped_boxplot_filtered.png}
...
The experimental results at this stage corroborate our initial hypothesis (\ref{h1}). As demonstrated by the stratified analysis in Figure~\ref{fig:success_rate_gates_by_payload}, the number of single-qubit gates has only a modest impact on success rates when controlling for payload size, even when applying thousands of gates. This pattern holds consistently across both quantum hardware platforms, demonstrating that the minimal gate count effect is a general characteristic rather than a platform-specific behavior.
...
\begin{tabular}{|l|c|c|}
    \hline
    \textbf{Statistic} & \textbf{IBM (depths 5--19)} & \textbf{Rigetti (depths 10--24)} \\
    \hline
    Sample Size & 171 & 60 \\
    \hline
...
The p-values provide critical evidence for the statistical significance of these correlations. In hypothesis testing, the p-value represents the probability of observing the given data, or more extreme, under a specific statistical model assuming no correlation exists \cite{Wasserstein02042016_2016}. IBM's p-value of $6.04 \times 10^{-28}$ (effectively zero) indicates a strong incompatibility between the data and the null hypothesis of no correlation, suggesting that the observed negative correlation is highly unlikely under the assumption of independence. Rigetti's p-value of $9.50 \times 10^{-6}$ (0.0000095) also falls well below the conventional significance threshold of 0.05, similarly indicating incompatibility with the null hypothesis. 

Statistical analysis reveals that the two platforms show significant negative correlations between circuit depth and success rate, reinforcing the conclusion that circuit depth affects performance prior to the dominant influence of payload size.
